...
export async function authorized(req) {
  ...
  const rows = await db()`SELECT s.user_id AS id,u.username FROM moonshot_user_sessions s JOIN moonshot_users u ON u.id=s.user_id WHERE s.token_hash=${digest(token)} AND s.expires_at>now()`;
  return rows[0] || null;
}
export async function newSession(res, userId) {
  const token = randomBytes(32).toString('hex');
  await db()`INSERT INTO moonshot_user_sessions(token_hash,user_id,expires_at) VALUES (${digest(token)},${userId},now() + interval '30 days')`;
